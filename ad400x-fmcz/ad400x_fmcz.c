...
struct ad400x_init_param ad400x_init_param = {
	/* SPI engine*/
	{
		...
		1,							// cs_delay
		SPI_ENGINE_CONFIG_CPOL | SPI_ENGINE_CONFIG_CPHA,  					    // spi_config
		50000000,                   // spi_clk_hz
...
int main()
{
	...
	uint32_t spi_eng_msg_cmds[4] = { CS_DEASSERT, SLEEP(10), TRANSFER_BYTES_R(2), CS_ASSERT};
	...
	if (SPI_ENGINE_OFFLOAD_EXAMPLE == 0) {
		...
		/* Offload example */
			...
			for(i = 0; i < dev->spi_desc->rx_length/4; i++) {
				*offload_data *= 2;
				data = *offload_data;
				if (*offload_data > 524287)
					data = *offload_data - 1048576;
				printf("ADC%d: %d\n", i, data);
				offload_data += 1;
			}
		}
